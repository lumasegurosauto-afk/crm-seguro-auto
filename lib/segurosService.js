...
// 4. ANEXAR APÓLICE (CORRIGIDO PARA LER O FILELIST DA TELA)
async function anexarApolice(arquivoPdf, apoliceId) {
  try {
    // CORREÇÃO: Extrai o arquivo físico correto caso ele venha como FileList ou Evento
    let arquivoReal = arquivoPdf;
    if (arquivoPdf?.target?.files) {
...
// 5. NOVA FUNÇÃO: BUSCAR LISTA DE CLIENTES DETALHADA (CORRIGIDO SELECT DE APÓLICES)
async function listarClientesCompleto() {
  try {
    const { data: clientes, error } = await supabase
      .from('clientes')
      .select('id, nome, cpf_cnpj, telefone, email, origem_lead, veiculos(marca_modelo, placa), apolices(url_pdf_apolice)')
