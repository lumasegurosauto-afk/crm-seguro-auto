...
export default function Cadastro() {
  ...
  // NOVO ESTADO: Armazena a classificação selecionada
  ...
  async function handleSalvarSeguro(e) {
    ...
    try {
      ...
      // Salva a classificação selecionada (Apólice, Cálculo ou Proposta) no campo 'status'
      const { data: proposta, error: errProposta } = await supabase
        ...
        .insert([{ 
          ...
          status: tipoDocumento // GRAVANDO O TIPO SELECIONADO
        }])
        .select().single()
      ...
      alert('🎉 Registro cadastrado com sucesso como ' + tipoDocumento + '!')
    ...
    <div style={{ padding: '30px', fontFamily: 'Arial, sans-serif', maxWidth: '800px', margin: '0 auto' }}>
      ...
      <form onSubmit={handleSalvarSeguro} style={{ display: 'grid', gap: '20px', marginTop: '20px' }}>
        ...
        <fieldset style={{ border: '2px solid #2563eb', borderRadius: '8px', padding: '15px', background: '#eff6ff' }}>
          <legend style={{ fontWeight: 'bold', color: '#2563eb', padding: '0 5px' }}>O que você está registrando agora?</legend>
          <select value={tipoDocumento} onChange={e => setTipoDocumento(e.target.value)} style={{ width: '100%', padding: '10px', fontSize: '15px', fontWeight: 'bold', borderRadius: '6px', border: '1px solid #cbd5e1' }}>
        ...
        <fieldset style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '15px', background: '#fff' }}>
          <legend style={{ fontWeight: 'bold', color: '#475569' }}>Dados do Cliente</legend>
        ...
        <fieldset style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '15px', background: '#fff' }}>
          <legend style={{ fontWeight: 'bold', color: '#475569' }}>Especificações e Valores</legend>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
        ...
        <fieldset style={{ border: '1px solid #cbd5e1', borderRadius: '8px', padding: '15px', background: '#fff' }}>
          <legend style={{ fontWeight: 'bold', color: '#475569' }}>Vigência / Controle</legend>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
