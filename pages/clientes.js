...
export default function ListaClientes() {
  ...
  return (
    <div style={{ padding: '30px', fontFamily: 'Arial', backgroundColor: '#f4f6f9', minHeight: '100vh' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '20px' }}><h1>👤 Gestão de Segurados</h1><a href='/'>← Voltar</a></div>
          ...
          <div style={{ background: '#fff', padding: '20px', borderRadius: '8px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>{clientes.map(c => (
                <tr key={c.id} style={{ borderBottom: '1px solid #eee' }}><td style={{ padding: '12px' }}><button onClick={() => setClienteSelecionado(c)} style={{ background: 'none', border: 'none', color: '#0070f3', fontWeight: 'bold', cursor: 'pointer' }}>{c.nome}</button></td><td style={{ padding: '12px' }}>{c.status}</td><td style={{ padding: '12px' }}><button onClick={() => abrirEdicao(c)} style={{ background: '#f59e0b', color: '#fff', border: 'none', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer' }}>✏️ Rápido</button></td></tr>
              ))}\</tbody>
            </table>
          </div>
          {clienteSelecionado && (
            <div style={{ background: '#fff', padding: '25px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', borderTop: '4px solid #0070f3' }}>
              <h3>📋 Ficha do Segurado</h3><p><b>Nome:</b> {clienteSelecionado.nome}</p><p><b>Documento:</b> {clienteSelecionado.cpf_cnpj}</p><p><b>Contato:</b> {clienteSelecionado.telefone || '-'} | {clienteSelecionado.email || '-'}</p><p><b>Veículo:</b> 🚗 {clienteSelecionado.veiculos?.marca_modelo || '-'} (Placa: {clienteSelecionado.veiculos?.placa || '-'})</p><p><b>Seguradora:</b> {clienteSelecionado.apolices?.seguradora || '-'}</p><p><b>Apólice Nº:</b> {clienteSelecionado.apolices?.numero_apolice || '-'}</p><p><b>Vigência:</b> {clienteSelecionado.apolices?.inicio_vigencia || '-'} até {clienteSelecionado.apolices?.fim_vigencia || '-'}</p><p><b>Financeiro:</b> Prêmio R$ {clienteSelecionado.valor_calculado} | Comissão R$ {clienteSelecionado.comissao_valor}</p>
              <div style={{ display: 'flex', gap: '10px', marginTop: '20px' }}><button onClick={() => abrirEdicao(clienteSelecionado)} style={{ flex: 1, padding: '10px', background: '#0070f3', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>✏️ Editar</button>{clienteSelecionado.apolices?.url_pdf_apolice ? <button onClick={() => setPdfVisualizacao(clienteSelecionado.apolices.url_pdf_apolice)} style={{ flex: 1, padding: '10px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>👁️ PDF</button> : <input type='file' accept='application/pdf' onChange={e => handleUploadTardio(e, clienteSelecionado.apolices?.id, clienteSelecionado.id)} /> }<button onClick={() => handleExcluirCliente(clienteSelecionado.id)} style={{ padding: '10px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>🗑️ Deletar</button></div>
            </div>
          )}
        </div>
      )}
      {clienteEdicao && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1000 }}>
          <form onSubmit={salvarDadosEditados} style={{ background: '#fff', padding: '25px', borderRadius: '8px', width: '100%', maxWidth: '450px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '15px' }}><label>Prêmio: <input type='number' step='0.01' value={clienteEdicao.valor_calculado} onChange={e => setClienteEdicao({...clienteEdicao, valor_calculado: e.target.value})} style={{ width: '100%' }} /></label><label>Comissão: <input type='number' step='0.01' value={clienteEdicao.comissao_valor} onChange={e => setClienteEdicao({...clienteEdicao, comissao_valor: e.target.value})} style={{ width: '100%' }} /></label></div>
      ...
      {pdfVisualizacao && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '20px' }}><div style={{ background: '#fff', width: '100%', maxWidth: '850px', height: '80vh', borderRadius: '8px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}><div style={{ background: '#1e293b', padding: '10px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: '#fff', fontWeight: 'bold' }}>📄 Visualizador</span><button onClick={() => setPdfVisualizacao(null)} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer' }}>Fechar ✕</button></div><iframe src={pdfVisualizacao} style={{ width: '100%', height: '100%', border: 'none' }} title='Leitor CRM' /></div></div>
      )}
    </div>
  );
}
}
t: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '20px' }}><div style={{ background: '#fff', width: '100%', maxWidth: '850px', height: '80vh', borderRadius: '8px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}><div style={{ background: '#1e293b', padding: '10px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: '#fff', fontWeight: 'bold' }}>📄 Visualizador</span><button onClick={() => setPdfVisualizacao(null)} style={{ background: '#ef4444', color: '#fff', border: 'none', padding: '5px 12px', borderRadius: '4px', cursor: 'pointer' }}>Fechar ✕</button></div><iframe src={pdfVisualizacao} style={{ width: '100%', height: '100%', border: 'none' }} title='Leitor CRM' /></div></div>
      )}
    </div>
  );
}
