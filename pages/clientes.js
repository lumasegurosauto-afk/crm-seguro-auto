...
export default function ListaClientes() {
  ...
  // FUNÇÃO DE EXCLUSÃO SEGURA (Apaga dependências antes para não travar o banco)
  async function handleExcluirCliente(id, nome) {
    const confirmar = window.confirm(`⚠️ ATENÇÃO: Tem certeza que deseja excluir permanentemente o segurado "${nome}" e todos os seus veículos, propostas e apólices vinculados?`);
    if (!confirmar) return;

    try {
      // 1. Deleta propostas e apólices vinculadas ao ID do cliente
      await supabase.from('apolices').delete().eq('cliente_id', id);
      await supabase.from('propostas').delete().eq('cliente_id', id);

      // 2. Deleta o registro principal do cliente
    ...
    <div style={{ padding: '30px', fontFamily: 'Arial, sans-serif', backgroundColor: '#f4f6f9', minHeight: '100vh' }}>
            ...
            <div style={{ background: '#fff', padding: '25px', borderRadius: '8px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', borderTop: '4px solid #0070f3' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginTop: '20px', borderTop: '1px solid #eee', paddingTop: '15px' }}>
                <div style={{ display: 'flex', gap: '10px' }}>
                  ...
                  {/* BOTÃO EXCLUIR REINTEGRADO NA FICHA LATERAL */}
                  <button onClick={() => handleExcluirCliente(clienteSelecionado.id, clienteSelecionado.nome)} style={{ padding: '10px 15px', background: '#ef4444', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>🗑️ Deletar Segurado</button>
                </div>
      ...
      {pdfVisualizacao && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.6)', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 2000, padding: '20px' }}>
          <div style={{ background: '#fff', width: '100%', maxWidth: '850px', height: '80vh', borderRadius: '8px', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

git add .
git commit -m "feat: reintegração do botão de exclusão de segurados na ficha lateral"
